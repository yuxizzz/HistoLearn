...
From the PCA visualizations in Step 2, we observe clear class separation, suggesting that a simple downstream classifier training on the embeddings may perform well. We first try training a k-NN classifier on PCA-reduced embeddings. The `train_model()` function performs PCA, keeps the specified number of components (`dr_k`), and fits the chosen classifier.
...
Next, we try logistic regression to see if it yields better performance. Logistic regression, also known as linear probing, is commonly used as downstream classifiers, as described in Chen et al. (2024).
